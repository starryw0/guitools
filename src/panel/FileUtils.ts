import { AmbientLight, Camera, Color, Light, Mesh, Object3D, PerspectiveCamera, PointLight, Texture } from "three";
...
export class FileUtils {
	...
	private static appendLight(data: string, light: Light) {
		data += `\n   // ${light.name} `;
	...
	private static appendModelPosition(data: string, obj: Object3D) {
		data += `\n   //${obj.name} position`;
	...
	private static appendModelRotation(data: string, obj: Object3D) {
		data += `\n   //${obj.name} rotation`;
	...
	private static appendModelScale(data: string, obj: Object3D) {
		data += `\n   //${obj.name} scale`;
		data += this.getFormatKV(`model_${obj.name}_scale`, obj.scale.x);
		return data;
	}

	private static appendMeshPosition(data: string, mesh: Mesh) {
		data += `\n   //${mesh.name} position`;
	...
	private static appendMeshRotation(data: string, mesh: Mesh) {
		data += `\n   //${mesh.name} rotation`;
	...
	private static appendMeshScale(data: string, mesh: Mesh) {
		data += `\n   //${mesh.name} rotation`;
	...
	private static appendCameraConfig(data: string, camera: Camera) {
	...
	static generateJSONFile(params: FileParams): string {
		var configMap: Map<string, any> = new Map();
		params.meshGuiExportParams.meshMap.forEach((value, key) => {
			var name = key;
			var mesh = value.mesh as any;
			if (mesh.material.roughness) {
			...
			if (mesh.material.map) {
				const tex = mesh.material.map as Texture;
	...
	private static getFormatKV(key: string, value: any): string {
		return `\n   this.configMap.set(\'${key}\', ${value});`;
	}

	static downFile(fileName: string, data: string) {
